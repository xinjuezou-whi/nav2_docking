  ...
  std::shared_ptr<tf2_ros::Buffer> tf)
{
  ...
  try {
    node->declare_parameter<std::vector<std::string>>("dock_plugins", std::vector<std::string>());
    docks_plugins = node->get_parameter("dock_plugins").as_string_array();
...
bool DockDatabase::getDockInstances(const rclcpp_lifecycle::LifecycleNode::SharedPtr & node)
{
  ...
  try {
    node->declare_parameter<std::string>("dock_database", std::string());
    dock_filepath = node->get_parameter("dock_database").as_string();
  ...
  try {
    node->declare_parameter<std::vector<std::string>>("docks", std::vector<std::string>());
    docks_param = node->get_parameter("docks").as_string_array();
