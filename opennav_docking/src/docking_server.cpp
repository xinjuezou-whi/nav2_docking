...
DockingServer::on_configure(const rclcpp_lifecycle::State & state)
{
  ...
  node->declare_parameter<double>("controller_frequency", 50.0);
  controller_frequency_ = node->get_parameter("controller_frequency").as_double();
  // controller_frequency_ = node->declare_or_get_parameter("controller_frequency", 50.0);
  node->declare_parameter<double>("initial_perception_timeout", 5.0);
  initial_perception_timeout_ = node->get_parameter("initial_perception_timeout").as_double();
  // initial_perception_timeout_ = node->declare_or_get_parameter("initial_perception_timeout", 5.0);
  node->declare_parameter<double>("wait_charge_timeout", 5.0);
  wait_charge_timeout_ = node->get_parameter("wait_charge_timeout").as_double();
  // wait_charge_timeout_ = node->declare_or_get_parameter("wait_charge_timeout", 5.0);
  node->declare_parameter<double>("dock_approach_timeout", 30.0);
  dock_approach_timeout_ = node->get_parameter("dock_approach_timeout").as_double();
  // dock_approach_timeout_ = node->declare_or_get_parameter("dock_approach_timeout", 30.0);
  node->declare_parameter<double>("rotate_to_dock_timeout", 10.0);
  rotate_to_dock_timeout_ = node->get_parameter("rotate_to_dock_timeout").as_double();
  // rotate_to_dock_timeout_ = node->declare_or_get_parameter("rotate_to_dock_timeout", 10.0);
  node->declare_parameter<double>("undock_linear_tolerance", 0.05);
  undock_linear_tolerance_ = node->get_parameter("undock_linear_tolerance").as_double();
  // undock_linear_tolerance_ = node->declare_or_get_parameter("undock_linear_tolerance", 0.05);
  node->declare_parameter<double>("undock_angular_tolerance", 0.05);
  undock_angular_tolerance_ = node->get_parameter("undock_angular_tolerance").as_double();
  // undock_angular_tolerance_ = node->declare_or_get_parameter("undock_angular_tolerance", 0.05);
  node->declare_parameter<int>("max_retries", 3);
  max_retries_ = node->get_parameter("max_retries").as_int();
  ...
  node->declare_parameter<std::string>("fixed_frame", std::string("odom"));
  fixed_frame_ = node->get_parameter("fixed_frame").as_string();
  // fixed_frame_ = node->declare_or_get_parameter("fixed_frame", std::string("odom"));
  node->declare_parameter<double>("dock_prestaging_tolerance", 0.5);
  dock_prestaging_tolerance_ = node->get_parameter("dock_prestaging_tolerance").as_double();
  // dock_prestaging_tolerance_ = node->declare_or_get_parameter("dock_prestaging_tolerance", 0.5);
  node->declare_parameter<double>("rotation_angular_tolerance", 0.05);
  rotation_angular_tolerance_ = node->get_parameter("rotation_angular_tolerance").as_double();
  ...
  try {
    node->declare_parameter<bool>("dock_backwards", false);
  ...
  node->declare_parameter<std::string>("odom_topic", std::string("odom"));
  ...
  node->declare_parameter<double>("odom_duration", 0.3);
  double odom_duration = node->get_parameter("odom_duration").as_double();
...
void DockingServer::rotateToDock(const geometry_msgs::msg::PoseStamped & dock_pose)
{
  ...
  while (rclcpp::ok()) {
    ...
    *command = controller_->computeRotateToHeadingCommand(
  ...
  Dock * dock, geometry_msgs::msg::PoseStamped & dock_pose, bool backward)
{
  ...
  while (rclcpp::ok()) {
    ...
    auto command = std::make_unique<geometry_msgs::msg::Twist>();
    if (!controller_->computeVelocityCommand(target_pose.pose, *command, true, backward)) {
