...
: tf2_buffer_(tf), fixed_frame_(fixed_frame), base_frame_(base_frame)
{
  ...
  node->declare_parameter<double>("controller.k_phi", 3.0);
  k_phi_ = node->get_parameter("controller.k_phi").as_double();
  // k_phi_ = node->declare_or_get_parameter("controller.k_phi", 3.0);
  node->declare_parameter<double>("controller.k_delta", 2.0);
  k_delta_ = node->get_parameter("controller.k_delta").as_double();
  // k_delta_ = node->declare_or_get_parameter("controller.k_delta", 2.0);
  node->declare_parameter<double>("controller.beta", 0.4);
  beta_ = node->get_parameter("controller.beta").as_double();
  // beta_ = node->declare_or_get_parameter("controller.beta", 0.4);
  node->declare_parameter<double>("controller.lambda", 2.0);
  lambda_ = node->get_parameter("controller.lambda").as_double();
  // lambda_ = node->declare_or_get_parameter("controller.lambda", 2.0);
  node->declare_parameter<double>("controller.v_linear_min", 0.0);
  v_linear_min_ = node->get_parameter("controller.v_linear_min").as_double();
  // v_linear_min_ = node->declare_or_get_parameter("controller.v_linear_min", 0.1);
  node->declare_parameter<double>("controller.v_linear_max", 0.25);
  v_linear_max_ = node->get_parameter("controller.v_linear_max").as_double();
  // v_linear_max_ = node->declare_or_get_parameter("controller.v_linear_max", 0.25);
  node->declare_parameter<double>("controller.v_angular_max", 0.75);
  v_angular_max_ = node->get_parameter("controller.v_angular_max").as_double();
  // v_angular_max_ = node->declare_or_get_parameter("controller.v_angular_max", 0.75);
  node->declare_parameter<double>("controller.slowdown_radius", 0.25);
  slowdown_radius_ = node->get_parameter("controller.slowdown_radius").as_double();
  // slowdown_radius_ = node->declare_or_get_parameter("controller.slowdown_radius", 0.25);
  node->declare_parameter<double>("controller.rotate_to_heading_angular_vel", 1.0);
  rotate_to_heading_angular_vel_ = node->get_parameter("controller.rotate_to_heading_angular_vel").as_double();
  // rotate_to_heading_angular_vel_ = node->declare_or_get_parameter(
  //   "controller.rotate_to_heading_angular_vel", 1.0);
  node->declare_parameter<double>("controller.rotate_to_heading_max_angular_accel", 3.2);
  rotate_to_heading_max_angular_accel_ = node->get_parameter("controller.rotate_to_heading_max_angular_accel").as_double();
  // rotate_to_heading_max_angular_accel_ = node->declare_or_get_parameter(
  //   "controller.rotate_to_heading_max_angular_accel", 3.2);
  node->declare_parameter<bool>("controller.use_collision_detection", true);
  use_collision_detection_ = node->get_parameter("controller.use_collision_detection").as_bool();
  // use_collision_detection_ = node->declare_or_get_parameter(
  //   "controller.use_collision_detection", true);
  node->declare_parameter<std::string>("controller.costmap_topic", std::string("local_costmap/costmap_raw"));
  costmap_topic = node->get_parameter("controller.costmap_topic").as_string();
  // costmap_topic = node->declare_or_get_parameter("controller.costmap_topic",
  //   std::string("local_costmap/costmap_raw"));
  node->declare_parameter<std::string>("controller.footprint_topic", std::string("local_costmap/published_footprint"));
  footprint_topic = node->get_parameter("controller.footprint_topic").as_string();
  // footprint_topic = node->declare_or_get_parameter("controller.footprint_topic",
  //   std::string("local_costmap/published_footprint"));
  node->declare_parameter<double>("controller.transform_tolerance", 0.1);
  transform_tolerance_ = node->get_parameter("controller.transform_tolerance").as_double();
  // transform_tolerance_ = node->declare_or_get_parameter(
  //   "controller.transform_tolerance", 0.1);
  node->declare_parameter<double>("controller.projection_time", 5.0);
  projection_time_ = node->get_parameter("controller.projection_time").as_double();
  // projection_time_ = node->declare_or_get_parameter(
  //   "controller.projection_time", 5.0);
  node->declare_parameter<double>("controller.simulation_time_step", 0.1);
  simulation_time_step_ = node->get_parameter("controller.simulation_time_step").as_double();
  // simulation_time_step_ = node->declare_or_get_parameter(
  //   "controller.simulation_time_step", 0.1);
  node->declare_parameter<double>("controller.dock_collision_threshold", 0.3);
  dock_collision_threshold_ = node->get_parameter("controller.dock_collision_threshold").as_double();
  ...
  const geometry_msgs::msg::Pose & target_pose, bool is_docking, bool backward)
{
  ...
  do{
    ...
    pose2d.x = local_pose.pose.position.x;
    pose2d.y = local_pose.pose.position.y;
    pose2d.theta = tf2::getYaw(local_pose.pose.orientation);
    if (use_collision_detection_ &&
      dock_collision_distance > dock_collision_threshold_ &&
      !collision_checker_->isCollisionFree(pose2d))
    {
      RCLCPP_WARN(
