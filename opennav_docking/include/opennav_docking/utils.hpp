  ...
  DockMap & dock_db)
{
  ...
  for (const auto & dock_name : docks_param) {
    node->declare_parameter<std::string>(dock_name + ".frame", std::string("map"));
    curr_dock.frame = node->get_parameter(dock_name + ".frame").as_string();
    // curr_dock.frame = node->declare_or_get_parameter(dock_name + ".frame", std::string("map"));

    try {
      node->declare_parameter<std::string>(dock_name + ".type", std::string());
      curr_dock.type = node->get_parameter(dock_name + ".type").as_string();
    ...
    pose_arr.clear();
    try {
      node->declare_parameter<std::vector<double>>(dock_name + ".pose", std::vector<double>());
      pose_arr = node->get_parameter(dock_name + ".pose").as_double_array();
    ...
    node->declare_parameter<std::string>(dock_name + ".id", std::string());
    curr_dock.id = node->get_parameter(dock_name + ".id").as_string();
